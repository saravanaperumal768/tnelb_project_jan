        ...
        <form id="competency_form_a" enctype="multipart/form-data">
            ...
            <div class="tabs" id="tabbedForm">
                ...
                <div class="tab" data-name="Basic Details">
                    ...
                    <!-- Input box for number of partners/directors -->
                    <div class="row mt-2" id="count-input-row" style="display:none;">
                        <div class="col-12 col-md-3 offset-md-2 text-md-right">
                            <label id="count-label"></label>
                        </div>
                        <div class="col-12 col-md-3">
                            <input type="number" class="form-control" id="count-input" min="1" />
                            <span class="error text-danger" id="count-input-error"></span>
                        </div>
                    </div>
...
let proprietorCount = initialDraftCount || 0;
    ...
    <script>
        ...
        $('#type_of_ea_license').on('change', function() {
            ...
            // Reset everything
            $('#count-input-row').hide();
            $('#partner-section, #director-section, #proprietor-sectionfresh').hide(); // hide all
            $('#partner-container, #director-container').empty();
            $('#count-input').val("");

            if (selected == "2") { // Partners
                $('#count-label').text("How many partners? (Min 2, Max 6)");
                $('#count-input').attr({
                    min: 2,
                    max: 6
                });
                $('#count-input-row').show();
                $('#partner-section').show();
            } else if (selected == "3") { // Directors
                $('#count-label').text("How many directors? (Min 2, Max 10)");
                $('#count-input').attr({
                    min: 2,
                    max: 10
                });
                $('#count-input-row').show();
                $('#director-section').show();
            } else if (selected == "1") { // Proprietor
        ...
        $('#count-input').on('input', function() {
            var selected = $('#type_of_ea_license').val();
            var count = parseInt($(this).val());
            var minRequired = 2;
            var maxAllowed = selected == "2" ? 6 : 10;

            // Validation
            if (count < minRequired) {
                $('#count-input-error').text("Minimum required is " + minRequired);
                $(this).val(minRequired);
                count = minRequired;
            } else if (count > maxAllowed) {
                $('#count-input-error').text("Maximum allowed is " + maxAllowed);
                $(this).val(maxAllowed);
                count = maxAllowed;
            } else {
                $('#count-input-error').text("");
            }
            ...
            for (var i = 1; i <= count; i++) {
                if (selected == "2") {
                    ...
                    $('#partner-container').append(partnerClone.replace("Partner Details", "Partner " + i + " Details"));
                } else if (selected == "3") {
                    var directorClone = $('#director-template').html();
                    $('#director-container').append(directorClone.replace("Director Details", "Director " + i + " Details"));
                }
            }
        });
        ...
        function addStaffRow() {
            let rowCount = $('#staff-container tr').length;
                ...
                Swal.fire({
                    ...
                    width:450,
                    title: 'Limit Reached',
                    text: 'You can add a maximum of 9 staff members.',
                    confirmButtonText: 'OK'
                });
                return;
            }
