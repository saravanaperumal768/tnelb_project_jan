    ...
    $(document).on('click', '.deleteMenu', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/admin/submenus/${menuId}/deactivatesubmenu`,
                    type: 'POST',
                    headers: { 'X-CSRF-TOKEN': csrfToken },
                    success: function (response) {
                        if (response.success) {
...
const submenuInsertRoute = "{{ route('admin.submenus.insertsubmenu') }}";
const submenuUpdateRoute = "{{ route('admin.submenus.updatesubitems') }}";
const submenuReorderRoute = "{{ route('admin.submenu.reorders') }}";
const newsboardUpdateRoute = "{{ route('admin.newsboard.update') }}";
const newsboardUpdateTamilRoute = "{{ route('admin.newsboard.updatetamil') }}";
