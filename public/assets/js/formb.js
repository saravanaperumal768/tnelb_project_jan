$("#contractor_form_b").on("submit", function (e) {
...
function submitFormeBFinalform(formData, actionType) {
// alert('eb form');
// exit;
    ...
    let postUrl =
        applType === "R"
            ? BASE_URL + "/formb/storerenewal"
            : BASE_URL + "/formb/storerecords";
...
// ----recent added---------------------
function verifyebCertificateprevoius(e, btn) {
    e.preventDefault();
    ...
    if (!prefixPattern.test(licenseNumber)) {
        resultBox.html(`<span class="text-danger">⚠️ License number must start with EB or L.</span>`);
        hiddenInput.val("0"); // invalid
        return;
    }

    // Normalize EA prefix
    licenseNumber = licenseNumber.replace(/^EB/i, "");
    resultBox.html(`<span class="text-info">Verifying...</span>`);

    $.ajax({
        url: BASE_URL + "/verifylicenseformeb_appl",
        method: "POST",
        data: {
            license_number: licenseNumber,
            date: date,
