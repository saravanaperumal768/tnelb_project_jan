$("#contractor_form_sa").on("submit", function (e) {
...
// console.log(p.ccverify, p.expverify);
    ...
    if (isValiddraft) {
        if (actionType === "draft") {
            submitFormAFinalforms(formData, actionType);
            return;
        }
    }
    // alert(actionType);
...
// ------------------ 3. Previous Contractor License ------------------
    ...
    if (!previousSelected) {
     ...
     else if (!/^ESA|LESA/i.test(prevAppNo)) {
    ...
    $("#previous_application_number").after(
        '<span id="previous_application_number_error" class="error text-danger d-block">License number must start with "ESA or LESA".</span>'
    );
...
// 3..............................
    ...
    if (isValid) {
        ...
        showDeclarationPopupformSA(formData);
    }
    // if (staffCount < 4) {
...
function showDeclarationPopupformSA(formData) {
    // alert('111');
    ...
    $.ajax({
        ...
        instructionsList.innerHTML = topDetails + html;
            ...
            proceedBtn.onclick = function () {
                ...
                submitFormAFinalforms(formData, "submit");
            };
...
function submitFormAFinalforms(formData, actionType) {
    ...
    let postUrl =
        applType === "R"
            ? BASE_URL + "/formsa/storerenewal"
            : BASE_URL + "/formsa/storerecords";
...
function showPaymentInitiationPopupformSA(
    application_id,
    transactionId,
    transactionDate,
    applicantName,
...
function showPaymentSuccessPopupformSA(
    application_id,
    transactionId,
    transactionDate,
    applicantName,
...
// ----recent added---------------------
function verifysaCertificateprevoius(e, btn) {
    e.preventDefault();
    ...
    $parent.find(".license-status").hide();
    const prefixPattern = /^(ESA|LESA)/i;
    ...
    if (!prefixPattern.test(licenseNumber)) {
        resultBox.html(`<span class="text-danger">⚠️ License number must start with ESA or LESA.</span>`);
        hiddenInput.val("0"); // invalid
        return;
    }

    // Normalize EA prefix
    licenseNumber = licenseNumber.replace(/^ESA/i, "");
    resultBox.html(`<span class="text-info">Verifying...</span>`);

    $.ajax({
        url: BASE_URL + "/verifylicenseformsa_appl",
        method: "POST",
        data: {
            license_number: licenseNumber,
            date: date,
